...
(function (global) {
    ...
    iSliderPrototype._setting = function () {
         ...
         * @public
         */
        self.defaultImage = opts.defaultImage || ''
        ...
        self.isLooping = opts.isLooping ? true : false;
    ...
    iSliderPrototype._renderItem = function (el, dataIndex) {
        ...
        var insertImg = function renderItemInsertImg() {
            ...
            if(item.content_type && item.content_type == 'video/mp4'){
                let imgsrc = 'https://cdn.uneed.com/file/play_icon.png'
                el.innerHTML = '<img class="islider-play-icon" src="'+ imgsrc +'"/>'
            }
        };

        var insertDefaultImg = function(error){
            el.style.backgroundImage = 'url(' + self.defaultImage + ')'
            if(error){
                el.innerHTML = ''
                return
            }
            let imgsrc = 'https://cdn.uneed.com/file/loading.png'
            el.innerHTML = '<img class="islider-loading-animation" src="'+ imgsrc +'"/>'
        }
        ...
        switch (type) {
            case _NT.pic:
                ...
                if (item.load === 2) {
                    ...
                    insertDefaultImg()
                    // console.log('没有预加载，需要重新加载了: ')
                    var currentImg = new Image();
                    currentImg.src = item.content;
                    currentImg.name = dataIndex
                    ...
                    currentImg.onerror = function (e) {
                        console.log('renderItem: img error: ', e)
                        item.load = 3
                        insertDefaultImg(true)
                    }
                }
                break;
            case _NT.dom:
    ...
    iSliderPrototype._renderWrapper = function () {
        ...
        for (var i = 0; i < 3; i++) {
            ...
            if(i === 0) continue;
            // this.isVertical && (this.animateType === 'rotate' || this.animateType === 'flip' || this.animateType === 'turn')
     ...
     * @private
     */
    iSliderPrototype._resetAnimation = function (type) {
        if (type) {
            this._animateFunc = this._animateFuncs[type]
        ...
        for (var i = 0; i < 3; i++) {
            ...
            els[i].style.zIndex = ''
            els[i].style['-webkit-backface-visibility'] = ''
            els[i].style.opacity = ''
    ...
    iSliderPrototype.resizeHandler = function () {
        if(this.width == this.wrap.offsetWidth) return //只有高度变动时，不触发resize
    ...
    iSliderPrototype.play = function () {
        ...
        if (this.isAutoplay && this.data.length > 1 && (this.isLooping || this.slideIndex < this.data.length - 1)) {
