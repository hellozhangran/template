    ...
    iSlider && iSlider.extend(iSlider._animateFuncs, {
        ...
        rotate: (function () {
            function rotate(dom, axis, scale, i, offset, direct) {
                ...
                iSlider.setStyle(dom, "position", "absolute")
                iSlider.setStyle(dom, "backface-visibility", "hidden")
        ...
        // turn 
        // turn: (function () {
        //     function turn(dom, axis, scale, i, offset, direct) {
        //         if (this.isVertical) {
        //             offset = -offset;
        //         }
        ...
        //         iSlider.setStyle(dom, "position", "absolute")
        //         iSlider.setStyle(dom, "backface-visibility", "hidden")
        //         iSlider.setStyle(dom, 'transform-origin', '0 100%');
        //         iSlider.setStyle(dom, 'transform', 'rotate' + ((axis === 'X') ? 'Y' : 'X') + '(' + 180 * (offset / scale + i - 1) + 'deg)');
        //     }

        //     turn.effect = iSlider.styleProp('transform');
        //     turn.reverse = true;
        //     return turn;
        // })(),
        depth: (function () {
        ...
        card: (function () {
            function card(dom, axis, scale, i, offset, direct) {
                ...
                dom.style.zIndex = z;
                iSlider.setStyle(dom, 'transform', 'translateZ(0) translate' + axis + '(' + ((1 + Math.abs(i - 1) * 0.2) * offset + scale * (i - 1)) + 'px) scale(' + zoomScale + ')');
            }
        ...
        zoomout: (function () {
            ...
            function zoomout(dom, axis, scale, i, offset) {
                ...
                iSlider.setStyle(dom, 'z-index', z);
                iSlider.setStyle(dom, 'opacity', o);
                iSlider.setStyle(dom, 'transform', 'scale(' + s + ')');
            }
